# homebrew apps
brew install ack atool cpulimit duff exiftool eye-d3 git git-extras git-ftp hr lftp mackup mediainfo mkvtoolnix mp4box namebench peerflix phantomjs shellcheck trash tree watchman z
...
# install apps
brew cask install --appdir="/Applications" airserver alfred apikitchen bartender bettertouchtool cocoadialog dropbox enjoyable flux fontprep google-chrome hex-fiend hydra imageoptim instacast iterm2 joinme keka key-codes leap-motion macaw macdown node-webkit openemu p5 platypus pokerstars processing screenflow shotcut spectacle steam sublime-text textexpander transmission

# drivers
brew cask install xbox360-controller-driver wacom-bamboo-tablet

# prefpanes and qlplugins
brew cask install betterzipql jsonlook qlcolorcode qlimagesize qlmarkdown qlstephen scriptql secrets suspicious-package
