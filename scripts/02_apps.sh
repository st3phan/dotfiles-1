install_brew_apps() {
  ...
  brew install aria2 chromedriver cpulimit duti eye-d3 ffmpeg geckodriver ghi git git-ftp handbrake haskell-stack hr httpie imagemagick jq mas massren mediainfo mkvtoolnix mp4v2 pup ripgrep rmlint shellcheck subliminal svgcleaner trash tree youtube-dl z
...
install_tinyscripts() {
  ...
  brew install annotmd cask-repair climergebutton contagem-edp fastmerge gfv gifmaker human-media-time labelcolor lovecolor macspoof pedir-gas pinboardbackup pinboardlinkcheck pinboardwaybackmachine podbook prfixmaster progressbar ringtonemaker seren trello-purge-archives
}
