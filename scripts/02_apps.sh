...
install_cask_apps() {
  ...
  brew cask install alfred appearin apple-events atom bartender bettertouchtool calibre dolphin dropbox electron-api-demos flux fog gifloopcoder gitup google-chrome imageoptim imitone iterm2 keka losslesscut oversight processing shotcut spectacle steam torbrowser transmission veertu-desktop whale whatsyoursign wwdc yacreader
...
install_tinyscripts() {
  ...
  brew install annotmd cask-repair contagem-edp customise-terminal-notifier fastmerge gfv gifmaker gifprogress human-media-time labelcolor lovecolor pedir-gas pinboardlinkcheck pinboardwaybackmachine podbook prfixmaster ringtonemaker seren trello-purge-archives
}
