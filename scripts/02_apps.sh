# homebrew apps
brew install aria2 asciinema atool ccat cpulimit duff duti exiftool eye-d3 ghi gifify git git-extras git-ftp hr httpie jq lftp mediainfo mkvtoolnix onepass phantomjs pup shellcheck the_silver_searcher trash tree youtube-dl z
...
brew cask install betterzipql colorpicker-skalacolor epubquicklook qlcolorcode qlimagesize qlplayground qlstephen quicklook-json suspicious-package ttscoff-mmd-quicklook
...
for ext in {css,html,jade,js,json,php,py,rb,sh}; do duti -s com.github.atom "${ext}" all; done # code
