install_brew_apps() {
  ...
  brew install aria2 asciinema asciinema2gif atool ccat cpulimit duff duti exiftool eye-d3 ghi gifify git git-extras git-ftp haskell-stack hr httpie jq lftp livestreamer mediainfo mkvtoolnix mp4v2 onepass phantomjs pup shellcheck subliminal the_silver_searcher trash tree wiki youtube-dl z
...
install_tinyscripts() {
  ...
  brew install annotmd cask-repair contagem-edp crafts customise-terminal-notifier fastmerge gifmaker gfv human-media-time labelcolor lovecolor pedir-gas pinboardlinkcheck podbook prfixmaster seren trello-purge-archives
}
