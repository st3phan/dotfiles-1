install_brew_apps() {
  ...
  brew install aria2 asciinema asciinema2gif chromedriver cpulimit duti eye-d3 ffmpeg ghi git git-ftp handbrake haskell-stack hr httpie imagemagick jq livestreamer mas mediainfo mkvtoolnix mp4v2 onepass pup ripgrep rmlint shellcheck subliminal svgcleaner trash tree youtube-dl z
...
install_cask_apps() {
  ...
  brew cask install alfred appearin apple-events atom bartender bettertouchtool blockblock calibre dolphin dropbox electron-api-demos fog gifloopcoder gitup google-chrome imageoptim imitone iterm2 keka oversight processing ransomwhere shotcut spectacle steam torbrowser transmission veertu-desktop whale whatsyoursign wwdc yacreader
